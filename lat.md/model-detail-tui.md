...
The merge logic distinguishes two arms over the inspect `Result`. `Ok(ModelDetail)` updates cache via `cache.write_models(...)` with new `metadata_kv_json` + `metadata_introspected_at` columns atomically per model_id — the cache writeback that AC-22-2 mandates. `Err(InspectError::Unsupported)` renders the `METADATA_UNSUPPORTED_SENTINEL = "(metadata unsupported for this tool)"` constant. Every other error variant (`FileReadable`, `FormatUnreadable`, `PluginPanic`) routes through the catch-all `Err(_)` arm to `INSPECT_PANIC_SENTINEL = "(inspection failed -- see diagnostics.log)"`; the panic-flavoured variants additionally emit an `inspect_panic tool=<id>` line to `diagnostics.log`.
...
The AC-22-7 (un-introspectable file) scenario routes through the Ollama plugin's production `inspect_model` override. With no manifest matching the synthetic id under `<MODELTAP_OLLAMA_DIR>/manifests/`, the locator returns `Err(InspectError::FileReadable)`, which the orchestrator's merge maps to `INSPECT_PANIC_SENTINEL = "(inspection failed -- see diagnostics.log)"` — the literal wording the source `.feature` line declares. AC-22-7's intent ("partial info gracefully" + "screen does not crash" + "other panels still render") is exercised end-to-end through that error arm.

The regression-gate test `ac_22_7_assertion_pins_inspect_panic_sentinel_literal` in [[tests/acceptance/regression_gate.rs]] pins the relationship between the production constant and the acceptance test's substring assertion — preventing the drift class that escaped review when the Ollama override first landed (test still asserted the old `METADATA_UNSUPPORTED_SENTINEL` after the live render had switched).

[[tests/src/fixtures/inspect_fixtures.rs]] ships `devon_model_unintrospectable_fixture`, which seeds a tempdir with an empty `manifests/` directory plus a non-GGUF binary file at `<temp>/ollama-root/unintrospectable-model.bin` (placed OUTSIDE `manifests/` so the locator's walk finds no match) plus the standard cache / log / diagnostics tree. The model file is reachable via `devon_unintrospectable_model_path(&fixture)`.

The headless lift that triggers `Msg::OpenDetail` on `<enter>` is the existing `MODELTAP_HEADLESS_DETAIL_REGS` JSON-payload seam in [[crates/modeltap-app/src/headless.rs]]'s `synthesize_detail_from_env` helper. The synthesiser's `tool` whitelist accepts `{ollama, hf, lm-studio}` only — the AC-22-7 driver uses `"ollama"` for its registration entry because the production Ollama plugin's `inspect_model` override is the merge branch under test.
