...
`MODELTAP_TEST_TOOL_INSPECT_PANIC=1` is the second per-method override. When set, both `TestTool::inspect_tool` and the in-binary `TestToolRegistration::inspect_tool` panic deliberately at the top of the function. The panic seam exists to drive end-to-end orchestrator panic-catch verification (INT-INFO-8 / AC-21-9 / AC-22-7) without modifying production plugin code. The orchestrator boundary at [[crates/modeltap-app/src/orchestration/open_tool_detail.rs]] wraps the call in `AssertUnwindSafe(...).catch_unwind()` and converts the unwinding into `Err(InspectError::PluginPanic)` plus a `diagnostics.log` line. See [[plugin-inspect-overrides]] "Panic isolation at the orchestrator boundary".
