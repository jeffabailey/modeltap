...
`MODELTAP_TEST_TOOL_INSPECT_PANIC=1` is the second per-method override. When set, both `TestTool::inspect_tool` and the in-binary `TestToolRegistration::inspect_tool` panic deliberately at the top of the function. The panic seam exists to drive end-to-end orchestrator panic-catch verification (INT-INFO-8 / AC-21-9) without modifying production plugin code. The orchestrator boundary at [[crates/modeltap-app/src/orchestration/open_tool_detail.rs]] wraps the call in `AssertUnwindSafe(...).catch_unwind()` and converts the unwinding into `Err(InspectError::PluginPanic)` plus a `diagnostics.log` line. See [[plugin-inspect-overrides]] "Panic isolation at the orchestrator boundary".

`MODELTAP_TEST_TOOL_INSPECT_MODEL_PANIC=1` is the third per-method override and the model-detail sibling of the INSPECT_PANIC seam. Landed in step 03-03 for the INT-INFO-8 / AC-22-7 cucumber test. When set, both `TestTool::inspect_model` (in [[tests/src/test_tool.rs]]) and the in-binary `TestToolRegistration::inspect_model` (in [[crates/modeltap-app/src/registry.rs]]) panic deliberately at the top of the function — placed BEFORE any normal logic (including the `id != TEST_MODEL_ID` short-circuit) so the panic fires for ANY model id the orchestrator might dispatch with. The env var is DELIBERATELY DISTINCT from `MODELTAP_TEST_TOOL_INSPECT_PANIC` so fixtures can target the two orchestrator boundaries (tool-detail vs model-detail) independently — setting the inspect_tool seam does NOT panic inspect_model and vice versa. The orchestrator boundary at [[crates/modeltap-app/src/orchestration/open_model_detail.rs]] handles the catch identically (same `AssertUnwindSafe(...).catch_unwind()` idiom) but writes a richer diagnostics line including the `model=<id>` field.
