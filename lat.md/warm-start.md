...
The `[cache]` section carries a third key, `persist_sha256` (US-27 / ADR-018), parsed by the same [[crates/modeltap-app/src/config.rs|`AppConfig`]] loader into `AppConfig.cache.persist_sha256`. Unlike `enabled` (opt-OUT, default true), this is opt-IN (default false): only when set does warm-start seed the in-process `Sha256Cache` from the persistent `cache_sha256` table (Tier 3) and the hash pool write computed hashes back. A missing/malformed config keeps it false, so SHA256 persistence never activates without explicit consent.

## Tier-3 SHA256 seed bridge (US-27)

[[crates/modeltap-app/src/sha256_persistence.rs]] is the composition-root bridge between the persistent `cache_sha256` table (Tier 3, owned by modeltap-store) and the RAM-only in-process [[crates/modeltap-app/src/sha256_cache.rs|`Sha256Cache`]] (Tier 1, which stays disk-free by its ADR-003 contract — persistence lives in the store, not in that type).

`seed_sha256_cache(cache, sha_cache)` reads every `cache_sha256` row via `Cache::all_sha256`, re-stats each path through [[crates/modeltap-store/src/revalidate.rs|`stat_file_quad`]] (the same quad-construction the revalidator uses, now `pub`), and for each row whose `(mtime,size,inode,dev)` quad still matches via `FileStat::matches`, lifts the stored hash into the in-process cache under the SAME key the pool uses — `Sha256CacheKey { path, mtime: secs, size }` per [[crates/modeltap-app/src/hash_pool_wiring.rs|build_hash_jobs]]. A matching seed means the background pool's `get_or_compute` HITS and never recomputes the unchanged file (AC-27-1). Quad-drifted rows are intentionally NOT seeded so the pool recomputes them (AC-27-2/3/4). The seed is best-effort: a cache-read error, a per-file stat error, or a corrupt hex hash skips that entry and never aborts launch.

`parse_hex_hash` / `hash_to_hex` are the 64-char lowercase-hex ⇄ `ContentHash([u8;32])` converters (the `cache_sha256.content_hash` column is hex TEXT). `writeback_hash(cache, path, hash)` is the best-effort write entry point the composition root calls on a freshly computed `Msg::HashComputed` when `persist_sha256` is on: it RE-STATS the path via `stat_file_quad` (full-precision `SystemTime` mtime) — NOT the hash job's second-granularity stat — so the next launch's seed quad-match (`FileStat::matches`) succeeds for an unchanged file, then upserts the `CachedSha256` row built by `build_writeback_entry`. ANY failure (file vanished, stat error, SQLite error) returns `false` without propagating (ADR-018 R10 — a writeback failure never blocks the user action).

### Production loop wiring + the `hash.computed` event

The hash pool worker tags each `Msg::HashComputed` with `was_computed` so the composition root can tell a real compute from a cache hit.

[[crates/modeltap-app/src/hash_pool/worker.rs]] `peek`s the in-process cache BEFORE computing: a hit (a hash seeded from Tier-3, or computed earlier this session) sets `was_computed = false`; an actual compute sets `true`. The pure `update` ignores the flag — the state transition is identical either way.

Both composition roots ([[crates/modeltap-app/src/interactive.rs]] and [[crates/modeltap-app/src/headless.rs]]) build a `persist_ctx = (store Cache, (tool,model_id)→path index)` BEFORE the pool spawns (gated on `persist_sha256`): they open the Tier-3 store via `open_store_cache`, `seed_sha256_cache` into the same `Sha256Cache` that is then moved into the pool (workers share its Arc-backed map, so the seed is visible), and snapshot the job paths (the jobs are consumed by `hash_pool::spawn`). At each hash-pool drain site, `observe_and_persist_hash` runs BEFORE the pure `update`: on a `was_computed == true` message it emits the `hash.computed` launch.log event ([[crates/modeltap-app/src/observability.rs|RecordKind::HashComputed]] — `{tool, model}`, privacy-preserving, NO path/hex) and writes the hash back to Tier-3. The event's ABSENCE for an unchanged file on the next launch is AC-27-1's observable proof that the seed skipped the re-hash.

`persist_sha256` is computed once in [[crates/modeltap-app/src/main.rs]] as `cache_enabled && app_config.cache.persist_sha256` and threaded into `interactive::run` + `HeadlessConfig`. When false (the default), none of the above runs and the launch path is byte-identical to pre-US-27.
