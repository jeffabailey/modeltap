...
`parse_hex_hash` / `hash_to_hex` are the 64-char lowercase-hex ⇄ `ContentHash([u8;32])` converters (the `cache_sha256.content_hash` column is hex TEXT). `build_writeback_entry` constructs the `CachedSha256` row the composition root upserts best-effort on `Msg::HashComputed` when `persist_sha256` is on (ADR-018 R10 — a writeback failure never blocks the user action).
