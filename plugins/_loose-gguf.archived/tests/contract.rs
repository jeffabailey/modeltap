//! Plugin contract tests for the loose-gguf plugin.
//!
...
//! a shared dev-dep because the suite is small. Verifies:
//!
//! - `name()` returns `"Loose GGUFs"` and is stable across calls.
...
use modeltap_plugin_loose_gguf::{LooseGgufPlugin, TOOL_NAME};

#[test]
fn name_is_loose_gguf_and_is_stable_across_calls() {
    let plugin = LooseGgufPlugin::new_with_search_paths(vec![PathBuf::from("/nonexistent")]);
    assert_eq!(plugin.name(), TOOL_NAME);
    assert_eq!(plugin.name(), ToolId("Loose GGUFs"));
...
fn accepted_formats_is_non_empty_and_stable_and_contains_gguf() {
    ...
    assert!(
        ...
        "loose-gguf must accept Gguf, got {:?}",
        first
    );
}
...
async fn discover_returns_not_installed_when_all_roots_missing() {
    let plugin = LooseGgufPlugin::new_with_search_paths(vec![
        PathBuf::from("/nonexistent/no-such-loose-gguf"),
        PathBuf::from("/nonexistent/no-such-loose-gguf-2"),
    ]);
...
async fn discover_is_idempotent_against_a_real_fixture() {
    ...
    let plugin = LooseGgufPlugin::new_with_search_paths(vec![temp.path().to_path_buf()]);
...
async fn discover_does_not_panic_on_unreadable_dir() {
    ...
    let plugin = LooseGgufPlugin::new_with_search_paths(vec![file]);
...
fn loose_gguf_plugin_is_present_in_inventory() {
    ...
    assert!(
        saw_loose_gguf,
        "loose-gguf plugin must self-register via inventory::submit!"
    );
}
