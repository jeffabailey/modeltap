...
pub struct LooseGgufConfig {
    pub search_paths: Vec<PathBuf>,
}

impl LooseGgufConfig {
...
pub struct Environment<'a> {
    /// Value of `MODELTAP_LOOSE_GGUF_DIRS`, if set.
    pub loose_gguf_dirs: Option<&'a str>,
...
/// Resolve the loose-gguf plugin's configuration from `env`. NEVER panics —
...
pub fn load_config(env: &Environment<'_>) -> LooseGgufConfig {
    let mut paths: Vec<PathBuf> = Vec::new();

    if let Some(raw) = env.loose_gguf_dirs {
...
pub fn from_process_env() -> Environment<'static> {
    ...
    let loose_gguf_dirs = std::env::var("MODELTAP_LOOSE_GGUF_DIRS")
        .ok()
        .map(leak_str)
        .map(|s| s as &str);
...
struct PluginsSection {
    #[serde(default, rename = "loose-gguf")]
    loose_gguf: Option<LooseGgufSection>,
}

#[derive(Debug, Deserialize)]
struct LooseGgufSection {
    #[serde(default)]
...
fn read_config_paths(path: &Path) -> Option<Vec<PathBuf>> {
    ...
    let doc: ConfigDoc = match toml::from_str(&raw) {
        ...
        Err(e) => {
            tracing::warn!(
                target: "modeltap.loose_gguf.config",
...
mod tests {
    ...
    fn config_toml_paths_used_when_env_absent() {
        ...
        std::fs::write(
            &cfg,
            r#"[plugins.loose-gguf]
search_paths = ["/data/models", "/srv/models"]
"#,
        )
        .unwrap();
        let env = Environment {
            loose_gguf_dirs: None,
