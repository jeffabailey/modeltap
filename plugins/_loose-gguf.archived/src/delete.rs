//! `Tool::delete_one` implementation for the loose-gguf plugin (US-05b, step 03-06).
//!
//! Loose GGUFs are plain `.gguf` files in user-convention directories. There is no manifest, no
...
/// Delete one loose-gguf model file at `target_path`.
///
...
pub fn delete_one_at(
    ...
    match std::fs::remove_file(target_path) {
        ...
        Err(e) => {
            tracing::warn!(
                target: "modeltap.loose_gguf.delete",
                "delete_one: remove {}: {e}",
                target_path.display()
            );
